...
Các thuật toán của garbage collector thường được chia thành các nhóm khác nhau dựa trên các đặc tính của chúng. Ở bài này thì mình sẽ giới thiệu cách chia dựa theo tính chất của chương trình mỗi khi thuật toán chạy:
* Stop-the-World: lý do nó có cái tên như thế là vì trong suốt quá trình chạy thu dọn vùng nhớ thì cả chương trình phải tạm dừng (suspend). Một vài thuật toán điển hình của nhóm này có thể kể ra như Naïve mark and Sweep và mark and compact mà chúng ta sẽ tìm hiểu chút nữa. Nhược điểm của nhóm thuật toán này đó là nó sẽ gây ra trì trệ lên hệ thống nếu quá trình dọn dẹp diễn ra thường xuyên hoặc quá lâu và khi đó người dùng có thể sẽ cảm thấy lag nhiều lần trong quá trình sử dụng. Một fun fact nhỏ đó là việc suspend của chương trình khi chạy thuật toán này đôi khi còn bị chọc là embarrassing pause.
* Incremental: nhằm khắc phục hiện tượng chương trình bị tạm dừng trong thời gian dài như trên thì các cụ não to đã nghĩ ra nhiều cải tiến mới cho các thuật toán và những thuật toán đời sau thường được gom nhóm vào incremental. Về bản chất, chương trình vẫn sẽ bị dừng. Nhưng, thời gian dừng mỗi lần chạy thuật toán là rất ít. Một trong số những cách nổi tiếng để cải thiện thời gian dừng đó là chia thuật toán thành nhiều phase chạy nhỏ hơn. Trong quá trình chạy các phase thì chương trình đúng là phải dừng lại, nhưng giữa các phase chạy với nhau thì chương trình được phép hoạt động trở lại và do đó sự ngắt quãng được giảm đi đáng kể.

### Naïve mark and Sweep

Một trong những thuật toán cơ bản và dễ hiểu nhất của tracing collector và nhóm stop-the-world là Naïve mark and sweep (đánh dấu và quét dọn). Naïve ở đây có thể hiểu theo nghĩa giống như chúng ta chạy thuật toán theo cách đơn giản nhất mà chúng ta cũng có thể tự làm được. Do đó nó mới dễ hiểu và ngoài ra cũng có một số biến thể cho thuật toán mark and sweep này.
...
Vừa rồi chúng ta đã đi qua 2 thuật toán của nhóm stop-the-world và các bạn có thể thấy là nó có nhiều hạn chế trong lúc vận hành do phải ngưng toàn bộ chương trình lúc đó. Như đã đề cập, để cải tiến thì người ta đã nghĩ ra cách chia thuật toán thành nhiều phase chạy khác nhau và cho phép chương trình chạy giữa các phase nhằm giảm thiểu thời gian ngắt quãng của chương trình. Phương pháp này còn được gọi là incremental và một trong số những thuật toán nổi tiếng cho phương pháp này là tri-color marking.

### Tri-color marking

Tri-color marking, tạm dịch là đánh dấu 3 màu sử dụng cơ chế chia các object vào các nhóm màu tương ứng nhằm cải thiện các vấn đề về hiệu năng ở nhóm thuật toán stop-the-world thông thường. Với phương pháp này, đầu tiên chúng ta sẽ có 3 màu sau:
